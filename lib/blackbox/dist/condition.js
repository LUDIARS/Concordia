...
export function evaluate(cond, features) {
    switch (cond.op) {
        case 'and':
...
function evalCmp(actual, op, expected) {
    if (actual === undefined)
        return false;
    if (op === '==')
 ...
 */
export function validateCondition(value, depth = 0) {
    if (depth > 8)
        throw new Error('condition nested too deep (max 8)');
    ...
    switch (c.op) {
        ...
        case 'or': {
            if (!Array.isArray(c.clauses) || c.clauses.length === 0)
                throw new Error(`${c.op} needs non-empty clauses`);
        ...
        case 'in': {
            if (typeof c.feature !== 'string')
                throw new Error('in.feature must be string');
            if (!Array.isArray(c.values) || c.values.length === 0)
                throw new Error('in.values must be non-empty array');
            const values = c.values.map((v) => {
                if (typeof v !== 'string' && typeof v !== 'number')
                    throw new Error('in.values must be string|number');
                return v;
            });
        ...
        case 'cmp': {
            if (typeof c.feature !== 'string')
                throw new Error('cmp.feature must be string');
            if (!CMP_OPS.includes(c.cmp))
                throw new Error(`cmp.cmp must be one of ${CMP_OPS.join(',')}`);
            const v = c.value;
            if (typeof v !== 'number' && typeof v !== 'string' && typeof v !== 'boolean') {
                throw new Error('cmp.value must be number|string|boolean');
            }
            return { op: 'cmp', feature: c.feature, cmp: c.cmp, value: v };
        }
        default:
