...
import { DEFAULT_THRESHOLDS, isLive, shadowPatch, verdictPatch } from './lifecycle.js';
export class BlackBoxEngine {
    ...
    firstLiveMatch(rules, features) {
        const live = rules
            .filter((r) => isLive(r.state))
            .sort((a, b) => b.priority - a.priority);
        for (const r of live) {
            if (evaluate(r.when, features))
                return r;
        }
        return null;
    }
     ...
     */
    async decide(domain, input, features, llmFallback) {
    ...
    runShadow(rules, features, teacherOutput, excludeId = null) {
        const evals = [];
        for (const r of rules) {
            if (r.state !== 'candidate' || r.id === excludeId)
                continue;
            if (!evaluate(r.when, features))
                continue;
     ...
     */
    recordVerdict(decisionId, verdict) {
        ...
        if (rec.source === 'llm' && verdict === 'ng') {
            for (const ev of rec.shadow) {
                ...
                if (!rule || rule.state !== 'candidate')
                    continue;
                // 教師 (LLM) が NG → 一致は誤った加点だったので衝突へ振り替え。
                // 不一致は「実は正しかったかもしれない」ので減点だけ取り消す (加点はしない)。
                const patch = shadowPatch(rule, ev.agreed ? { agreements: -1, conflicts: 1 } : { conflicts: -1 }, this.t);
                if (patch)
                    this.rules.update(ev.ruleId, patch);
            }
        }
        return { ok: true };
    }
