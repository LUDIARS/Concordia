...
export class JsonFileRuleStore {
    file;
    now;
    ...
    update(id, patch) {
        ...
        if (idx < 0)
            return null;
        const next = { ...state.rules[idx], ...patch, updatedAt: this.now() };
        state.rules[idx] = next;
        this.file.save(state);
        return { ...next };
    }
}
export class JsonFileDecisionLedger {
    file;
    maxDecisions;
    constructor(file, opts = {}) {
        this.file = file;
...
export function makeFileBlackBox(path, opts = {}) {
...
//# sourceMappingURL=store-jsonfile.js.map