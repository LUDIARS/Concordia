...
export function resolveLibraryRoots(workspaceRoots) {
    ...
    const centralSlug = (process.env.CONCORDIA_CENTRAL_MEMORY_SLUG ?? "").trim() || DEFAULT_CENTRAL_SLUG;
    const arsOk = arsDir !== "" && existsSync(arsDir);
    const projectsOk = existsSync(claudeProjectsDir);
    if (!arsOk && !projectsOk) {
        throw new LibraryRootsError(`走査 roots を解決できません: arsDir=${arsDir || "(未設定)"} / ` +
