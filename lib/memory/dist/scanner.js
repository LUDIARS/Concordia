...
import { parseMemoryIndex, parseFrontmatter } from "./parser.js";
import { scanPoison } from "./heuristics.js";
const MEMORY_INDEX = "MEMORY.md";
export const ARCHIVE_DIRNAME = "_archive";
function readFileStat(abs) {
    let st;
    try {
        st = statSync(abs);
    }
...
export function scanMemorySources(roots) {
    const out = [];
...
function buildMemorySource(slug, memoryDir, indexPath, central) {
    const indexStat = readFileStat(indexPath);
    const indexEntries = indexStat ? parseMemoryIndex(indexStat.content) : [];
    const entryByFile = new Map();
    for (const e of indexEntries)
        entryByFile.set(e.fileName, e);
...
export function scanSkillSources(roots) {
    const out = [];
    if (roots.arsDir) {
...
function buildSkillSource(sourceId, label, sourceKind, skillsDir) {
    const blocks = [];
    let entries = [];
    try {
...
function pushSkillBlock(blocks, sourceId, skillsDir, name, relPath, contentAbs) {
...
export function scanLibrary(roots) {
