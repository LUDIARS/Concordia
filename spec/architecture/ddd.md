...
サービス実装には Node.js / TypeScript を採用する。応答性、仕事の永続化、障害分離、停止・復旧の
規範は [Node.js 採用とサービス原則](nodejs-service-principles.md) を参照する。

2026-09-07の存在意義の明確化により、Ccは「全てのAIと人間が調和する空間」を提供する
ハーモナイザーとして設計する。以下の既存ドメイン分類は移行の出発点として維持するが、
コアの候補を「意図・現在地・関心を共有する協働」と「役割・権限・合意に基づく参加」の
2つの業務能力から再検討する。これは分類案であり、新しいコード所属の確定宣言ではない。

| 提案された機能境界 | UXから見た位置付け案 |
|---|---|
| AI管理（ハーネス） | 参加と協働のルールを実現する部分はコア候補。実行遮断やhook配送は実現手段 |
| 作業環境 | 協働を継続・復旧できる環境を提供する支援境界 |
| 権限 | 誰が何を決められるかという合意はコア候補。認証・権限チェックの基盤は支援部分 |
| 連携 | Discord等への配送は支援境界。会話と関心の共有体験は協働のコア候補に関係する |

機能ごとの分割とコア/支援の分類は同一ではない。AIによるつぶやき・会話も、協働する主体が
互いの関心を知るUXとして扱い、単なる通知の増量にしない。消費制御と参加の選択を併せて設計する。
