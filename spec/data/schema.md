...
| `taskflow_task_state` | task Markdown に対応する mutable runtime state | repo_path / task_path (複合 PK) / status (4 値制約) / subsidiary_id (`NULL`=本社) / source_session / assignee / owner / delegation_run_id / pr_number / memoria_task_id / actio_task_id / memoria_registration_state (3 値制約、created と memoria_task_id を整合)。Markdown は static definition のみ。 |
