...
`KAIZEN_DAILY_PROMPT` がその回のAstraに難易度の判断を指示する。
高難度は自身で実装、低難度はプロジェクトに適した実装モデルへ委託する。
判断に困るものはタスク化し、判断待ちをpartialの残作業として報告する。
人間の応答や終了許可を待って居残らず、既存の報告→status→退勤を利用する。
人間判断待ちの `remaining` は `kind: "wait"` を必須とする。Ccのpartial処理は
この項目を実装残から除外し、判断待ちだけなら次のrunを起動せず、その回のrunを閉じる。
タスクはpendingのままであり、runの終了を人間判断や改善実装の完了と扱わない。
共通injectに別の終了手順を追加しない。難易度によって権限・安全条件を緩めない。
仕様の正本は [delegation](delegation.md)。UX-CC-AD-W1、CC-INV-02/04/08に対応する。
