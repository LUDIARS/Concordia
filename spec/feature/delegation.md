...
2026-09-09 neco指示で、検知だけで終わらず実行可能な改善の実装とサマリ報告までを仕事と明記する。
日報・メール報告も実作業とsystemへの報告までを担当する。配送と終了の共通契約は
[パートタイマーinject](delegation-parttimer-inject.md)を参照する。
ユーザーがPR作成までと明示した回は、下記のマージ完了契約より優先して委託先にも伝え、提出後に停止する。
...
請求番号・請求日・対象月マーカーの更新規則は、設定
`delegation.invoice_skill_command` が指すスキルを正本とし、テンプレート側に複製しない
(二重管理を避ける)。コマンド名は英数字・underscore・hyphen の形式だけを受け付け、未設定・
形式不正なら書式を推測せず未実施として報告する。設定変更時は対象の seed テンプレートへ
即時反映する。金額と摘要は前月据え置きが既定。
...
脆弱性対応・レビュー・調査のように **コードを書かない設計** のテンプレは、成果物が
`Review/` への保存や報告であって feature branch ではない。この検証は branch 証跡を
要求するので、そうしたテンプレは恒常的に completed を報告できなかった。

テンプレートに `review_only` を持たせ、宣言のある run はこの検証の対象外にする。
判定は **テンプレート定義** から引く。run 側の状態 (branch が無い) から推測すると、
「実装 run が branch を持たないまま completed を自己申告する」という、この検証が
塞いでいる穴がそのまま開くため。テンプレートが削除済みで宣言を確認できない run は、
証跡を要求する側へ倒す。
