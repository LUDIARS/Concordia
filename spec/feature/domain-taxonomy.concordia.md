...
Agent delegation, persona generation, subsidiary bots, harness sessions, and advisory testing claims that coordinate work beyond a single session.

- **personas** - Dynamic persona seeds, signal collection, and persona feedback loops (src/personas).  `paths: (^|/)src/personas/[^/]+$`
- **delegation-coordination** - Delegation templates, parent/child run tracking, portable prompts, and injected coordination messages (src/delegation).  `paths: (^|/)src/delegation/[^/]+$`
...
- **session-lifecycle** - Session registration, heartbeat, end flow, spawn, compaction, and reaper control (src/control).  `paths: (^|/)src/control/[^/]+$`
...
- **bootstrap-composition** - Core bootstrap composition, chat/cost mode selection, service startup, and shutdown wiring (src/bootstrap).  `paths: (^|/)src/bootstrap/[^/]+$`
