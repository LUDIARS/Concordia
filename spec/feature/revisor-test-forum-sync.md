...
- 終局状態を確認できた候補は、最終詳細を取得・掲載してから投稿を閉じ、関連するテストセッションも終わらせる。
  終局一覧で **merged** と確認できる場合は、archive 前に「マージしました」を
  スレッドへ通常メッセージで残す。統合コミットのハッシュは投稿に出さず、Revisor の記録で照合する。終局一覧・個別詳細・投稿が失敗した場合は
  開いたまま保持し、次周期に再試行する。
...
- 投稿は人間向けの日本語で書く。Revisor が JSON 文字列で保存した記録 (保存済みの過去分を含む) は
  Cc が値へ戻し、Revisor 画面 (判定・マージリスクの内訳・動作確認・テスト表・レビュー) と同じ語で
  組み立てる。生 JSON、機械キーの列挙、審査 attempt・コミット・配送鍵のハッシュ、受領印の
  footer は投稿に出さない。
  - レビュー開始は開始した旨、各段階は開始・スキップ理由・結果、最終記録は結論・ブロック理由・
    所見・登録チェック・セキュリティ・レビュー本文を書く。通過した審査でも内容を省略しない。
    同じ attempt のモデルレビュー段階が同一本文を掲載済みなら、最終記録はその投稿を案内する。
  - 「審査結果・判断事項」は、マージリスクの点数と内訳 (加点要因と点数)、動作確認の判定と内訳、
    登録チェックの通過・失敗・スキップ全件 (名前・結果・exit code・所要・備考・失敗出力) と
    判断が必要な理由を並べる。
  - Revisor が返さなかった項目は「未取得」、レビュー本文を保存する前の記録は「本文は未取得」と
    明記し、加点なしや成功として補わない。壊れた記録・未対応の形式・伏字で読めない記録は
    表示できない旨だけを書き、断片を出さない。
- 旧 Revisor で履歴が無い場合はその制限を明記し、取得できたチェック結果・出力を掲載する。
  不正な v1 履歴は未取得扱いで黙って捨てず、読取エラーとする。
- 配送の同一性は PR・履歴 id・元データの指紋から作る内部の鍵とし、表示文言の改善で再送しない。
  受領は Cc の台帳 (`discord_review_report_receipts`、migration 107) が正本で、投稿には載せない。
  - 送信直前に pending を書き、送信の応答で delivered へ進める。応答を失った pending は、
    送信時刻以降の Bot 投稿と本文・添付全文が一致するかで照合し、一致しなければ再送する。
    Discord の nonce は直後の再送抑止にだけ使い、それだけを受領の根拠にしない。
  - 2026-09-12 版の footer 受領印 (`Revisor report • 鍵`) は、スレッドを初めて扱うときだけ読み、
    旧版と同じ式で再計算した鍵が揃う文書を配送済みとして取り込む。取り込んだ受領と完了印
