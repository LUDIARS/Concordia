...
- 認証にはsecret managerから注入した `CONCORDIA_REVISOR_WORKFLOW_TOKEN` を使う。
  Revisorのlocal workflow tokenと一致させ、PR-gate origin tokenとは混同しない。
...
接続は読取専用で、local workflow tokenをBearer送信する。
