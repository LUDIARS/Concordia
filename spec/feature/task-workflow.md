...
- **テストワークフローは現状のまま** (§4 テスト隔離 / §5 confirm キュー)。
  変更しない。
- **レビューとテストは作業完了後に非同期で実施してよい。**
  PR 作成をレビュー・テスト完了でブロックしない (直列待ちにしない)。
...
2. **worktree ではサービス起動・動作テストをしない。** 単体テスト (vitest 等、 起動を伴わない) は可。
   動作テストは Cc の confirm キューが安定ブランチで行う — 自分で確かめようとしない。
3. 実装が終わったら commit + PR + `POST /v1/delegation/runs/:id/status {completed}` まで。
   **テスト・マージ・確認はスコープ外** (Cc と人間が行う)。

## 4. テスト隔離 — 決定論ハーネス (LLM 判断なし)

「動作テスト」= サービス起動・実データ・手動確認を伴う確認。 単体テスト (vitest / jest 等、
起動を伴わずリポ内で完結) は含まない — CI と同等のため worktree 内でも可。
