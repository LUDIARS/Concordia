...
description: "チームごとに 1 本ずつ起動する Timer Delegation を 2 本追加する。朝礼 (毎朝 9:30) はチームの稼働状況と対応状況を 目標 面へカードとして報告し、台帳は書き換えない。定例 (火・金 13:00) はタスクの棚卸し議題を提示して neco の返信を待ち、Memoria タスクと director case step へ反映するまでを 1 回とする。既存 cron が持たなかった「チームごとの fanout」を scheduler に追加する。"
service: concordia
domain: governance
tags:
  - teams
...
Memoria タスク / PR / Excubitor の状態を読んで束ねるだけで、朝礼は書き換えを一切しない。
書き換えが起きるのは定例で、**neco が決めたことだけ**を反映する。
