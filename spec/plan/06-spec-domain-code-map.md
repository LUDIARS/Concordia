...
| C-05 chat is an adapter | chat-platforms, runtime-orchestration | `src/chat-worker.ts`, `src/platform/lifecycle.ts`, Discord/Slack routing, projection, and interaction ports | `spec/plan/process-isolation-v2.md` | implemented; adapter boundaries enforced |
...
| C-10 stable interfaces through isolation | http-interface, runtime-orchestration | `src/api/route-groups.ts`, purpose-specific CoreDeps, `src/bootstrap/boot-phases.ts`, `resource-owner.ts` | API tests, process isolation specs | implemented with explicit ownership |
