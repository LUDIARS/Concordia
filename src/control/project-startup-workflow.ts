...
export function renderProjectStartupWorkflow(workflow: ProjectStartupWorkflow): string {
  switch (workflow) {
    ...
    case "github":
      return "- Cc 判定: GitHub Workflow。既定の完了範囲は commit・作業 branch の push・GitHub PR 作成までです。Rv登録・GitHub Appはpushの必須条件ではありません。Gitの認証と既存のbranch保護を確認し、PR 作成後は停止してください。";
    default:
