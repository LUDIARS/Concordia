  ...
  upsertDelegation(subsidiaryId: string, input: UpsertOwnedDelegationInput): SubsidiaryDelegationRow {
    ...
    if (cur) {
      ...
      `).run(
        ...
        applyDelegationProviderPolicy(input.target_provider ?? cur.target_provider),
      ...
      `).run(
        ...
        input.title ?? "", input.description ?? "", applyDelegationProviderPolicy(input.target_provider ?? "claude"),
