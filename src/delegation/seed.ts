...
const SEED_TEMPLATES: CreateTemplateInput[] = [
  ...
  // task_list を引数として本テンプレを invoke する。
  {
    ...
    description: "Memoriaの今日期限タスクから、AIが実行可能なものを自動で処理する朝ルーティン。MorningSchedulerが毎朝8時にinvokeする。",
    target_provider: "claude",
    model: "claude-sonnet-4-6",
    emoji: "🌅",
    prompt_template: [
      ...
      "AIが実行可能なものを優先度順に **最大3件** 選んで対応してください。",
      "",
      "### 今日の対象タスク",
      "",
      "${task_list}",
      "",
      "### 対応基準",
      "- **実施する**: コード修正・実装, CLIコマンド実行 (`npm run ...` 等), PR作成, ステータス更新",
      "- **スキップ**: 実機確認, ブラウザ操作, Discord/GCP/AWS外部設定, データ手動入力, 「判断待ち」「指示待ち」と明記されているもの, 物理操作必須のもの",
      "",
      "### 手順",
      "1. 上記タスクから優先度・インパクト順に最大3件を選定",
      "2. **軽微な修正** (CORS追加・パス修正・設定変更等) は直接worktreeで実施してPR作成",
      "3. **複雑な実装** (新機能・大規模リファクタ) はAgent tool (model=opus) に委託し、自分はPR作成まで確認",
      "4. **CLIコマンド実行** タスク (`npm run companies:xxx` 等) は `--limit 100` 程度で実施",
      "5. 完了したタスクは `node \"E:/Document/Ars/.claude/skills/memoria-task/mmtask.mjs\" --set <id> done` でクローズ",
      "6. 全タスク処理後に結果サマリ（対応済み/スキップ一覧・理由）を投稿して `/session-end`",
      "",
      "注意: 破壊的なDB操作・本番への影響が不明な操作はスキップ。",
    ].join("\n"),
    input_schema: [
