...
const SEED_TEMPLATES: CreateTemplateInput[] = [
  ...
  // task_list を引数として本テンプレを invoke する。
  {
    ...
    description: "Memoriaの今日期限タスクを「確認系(人間がやる)」と「実装系(AIがやれる)」に仕分け、確認系は整理して提示し、実装系は残作業がなくなるまで実装する朝ルーティン。人間の判断が必要になったらaskで止める。MorningSchedulerが毎朝8時にinvokeする。",
    target_provider: "claude",
    model: "claude-sonnet-4-6",
    emoji: "🌅",
    prompt_template: [
      ...
      "5. 実装系の残作業がなくなるか、全タスクが「完了 or 確認系として提示済み or 判断待ちで保留」のいずれかになったら、結果サマリを投稿する。",
