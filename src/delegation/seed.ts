...
const SEED_TEMPLATES: CreateTemplateInput[] = [
  ...
  },
  {
    ...
    title: "チーム朝礼 (毎朝)",
    description: "チームごとの稼働状況と対応状況をまとめ、チームの 目標 面へカードとして投稿する。報告のみで台帳は書き換えない。2026-08-17 neco 指示で新設。",
    target_provider: "claude",
    model: "claude-sonnet-5",
    category: "parttimer",
    emoji: "🌅",
    prompt_template: [
      ...
      "チームの 目標 面へ 1 枚のカードとして投稿する回です。**調査と報告だけ**を行います。",
      ...
      "6. まとめを Concordia `POST /v1/teams/${team_id}/cards` へ投稿する。",
      "   body は `{\"kind\":\"standup\",\"title\":\"朝礼 ${date} — ${team_name}\",\"body\":\"<Markdown 本文>\"}`。",
      "   日本語ペイロードは JSON ファイル経由で POST する (シェルへ直接埋め込むと文字化けする)。",
      "7. API の受理結果と本文の要約を最終報告に載せる。実際の Discord 投稿は非同期なので、受理結果から配信済みとは断定しない。",
      "",
      "### カード本文の型 (4000 字以内)",
      ...
      "",
      "### やらないこと",
      "- step / タスク / PR の状態を書き換えない (朝礼は報告のみ。台帳の整理は定例で人間と決める)。",
