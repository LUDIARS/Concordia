...
vi.mock("../cost/context-estimate.js", () => ({
...
describe("formatContextReport", () => {
  it("占有 / 残量 / しきい値までの余裕を整形する", () => {
    const report = formatContextReport(estimate, 0.75, null);
  ...
  it("前回コンパクション時刻を ISO で表示する", () => {
    ...
    expect(formatContextReport(estimate, 0.75, at)).toContain("前回コンパクション: 2026-08-14T03:00:00.000Z");
  });

  it("窓超過時も残量・余裕を 0 に clamp する", () => {
    const over: ContextEstimate = { tokens: 250_000, windowTokens: 200_000, pct: 1 };
    const report = formatContextReport(over, 0.75, null);
...
describe("buildContextReport", () => {
  ...
  it("推定不能なら fallback メッセージを返す", async () => {
    ...
    expect(await buildContextReport(session)).toContain("推定できませんでした");
  });
  ...
  it("推定値と metadata.last_compaction_at から整形する", async () => {
    ...
    const report = await buildContextReport({ ...session, metadata: JSON.stringify({ last_compaction_at: at }) } as SessionRow);
  ...
  it("CONCORDIA_COMPACTION_AUTO_PCT がしきい値表示に反映される", async () => {
    ...
    vi.mocked(estimateContextTokens).mockResolvedValue(estimate);
    const report = await buildContextReport(session);
