 ...
 *
 * Excubitor から集約 (2026-05-17). Concordia の startBackend() から `bootObservability()`
 * を呼ぶと:
 *   - catalog 読み込み + DB 同期
 *   - default error rules seed
 *   - process bridge (spawn 子プロセスの stdout を log bus へ流す)
 *   - error detector (log bus を購読してパターン検知 → error_tasks 投入)
 *   - scanner loop (docker / git / package version の周期スキャン)
 *   - catalog watcher (services.yaml 変更検知して再 sync)
 *   - autostart 実行
 *   - HTTP router を返す (app.ts でマウント)
 */
...
const logger = pino({ name: 'concordia.observability' });
...
export async function bootObservability(): Promise<ObservabilityHandle> {
  ...
  // reviews router (path 内側で /api/v1/reviews を持つ ので root mount)
