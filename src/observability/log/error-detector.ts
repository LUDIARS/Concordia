...
async function reloadRules(): Promise<void> {
  ...
  for (const raw of rows as unknown as Array<Record<string, unknown>>) {
    try {
      ...
      // service_codes は SQLite では JSON 文字列で持っている (元 TEXT[])。
      // raw SQL 経由で取得しているので自前で JSON.parse する。
...
async function recordHit(rule: Rule, line: LogLine): Promise<void> {
  ...
  // SQLite では PG の data-modifying CTE (WITH ... UPDATE ... FROM si) を
  // 直接書けないので、 同一トランザクションで以下の手順に分解する:
  //   1) service_instance_id を resolve
  //   2) 60 秒以内の同 rule × instance × open 系 task を UPDATE ... RETURNING
  //   3) ヒット 0 件なら INSERT
  ...
  if (existing.length === 0) {
    ...
    db().run(sql`
      INSERT INTO error_tasks (id, rule_id, service_instance_id, severity, summary, log_excerpt)
      VALUES (${newId}, ${rule.id}, ${siRow.id}, ${rule.severity}, ${summary}, ${line.line})
    `);
  }
