...
describe("parseLocalPrDetail", () => {
  it("extracts the decision summary from a Revisor local PR", () => {
    ...
    expect(detail).toEqual({
      ...
      reviewReport: null,
      checks: [
        { name: "unit", status: "passed", exitCode: null, durationMs: null, reason: null, output: null },
        { name: "lint", status: "failed", exitCode: 1, durationMs: null, reason: "lint error", output: { text: "masked failure output", truncated: true } },
        { name: "e2e", status: "skipped", exitCode: null, durationMs: null, reason: null, output: null },
      ],
      author: "neco",
      headRef: "feat/x",
      baseRef: "main",
