...
describe("findCodexTranscript", () => {
  ...
  it("先頭行の session id で transcript を見つける", () => {
    const expected = writeRollout("15", "rollout-2026-07-15T10-00-00-a.jsonl", sessionMetaLine(SESSION_ID));
    expect(findCodexTranscript(SESSION_ID, { root })).toBe(expected);
  });

  it("巨大ファイルでも先頭チャンクだけで判定できる (全量読みしない)", () => {
  ...
  it("新しい日付ディレクトリを先に探す", () => {
    ...
    expect(findCodexTranscript(SESSION_ID, { root })).toBe(newest);
  });

  it("発見済み path はキャッシュされ、 次回はスキャンしない", () => {
    const expected = writeRollout("15", "rollout-2026-07-15T10-00-00-a.jsonl", sessionMetaLine(SESSION_ID));
    expect(findCodexTranscript(SESSION_ID, { root })).toBe(expected);
    // root を存在しないパスに差し替えてもキャッシュ済みなら返る。
    expect(findCodexTranscript(SESSION_ID, { root: join(root, "does-not-exist") })).toBe(expected);
  });

  it("見つからない sessionId は TTL 内は再スキャンしない (負キャッシュ)", () => {
    let clock = 1_000_000;
    const now = (): number => clock;
    expect(findCodexTranscript(SESSION_ID, { root, now })).toBeNull();
    ...
    expect(findCodexTranscript(SESSION_ID, { root, now })).toBe(path);
  });

  it("root が無ければ null", () => {
    expect(findCodexTranscript(SESSION_ID, { root: join(root, "missing") })).toBeNull();
  });
});
