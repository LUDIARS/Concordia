...
function readUsageStub(s: SessionRow): { total: number } | null {
...
describe("SubsidiaryBudgetTracker", () => {
  it("当日の subsidiary_id 一致セッションのトークンだけ合算する", () => {
    ...
    expect(tracker.todayTokens("sub-a", NOW)).toBe(30_000);
    expect(tracker.todayTokens("sub-b", NOW)).toBe(30_000);
    expect(tracker.todayTokens("sub-none", NOW)).toBe(0);
  });

  it("前日に始まったセッションは当日消費に含めない", () => {
    const [todayStart] = localDayRange(NOW);
    addSession("s10", todayStart - 1000, "sub-a"); // 昨日
    addSession("s20", NOW, "sub-a"); //              今日
    const tracker = new SubsidiaryBudgetTracker({ sessionsRepo: repo, readUsage: readUsageStub, now: () => NOW });
    expect(tracker.todayTokens("sub-a", NOW)).toBe(20_000);
  });

  it("status: budget=0 は無制限 (blocked=false)", () => {
    addSession("s500", NOW, "sub-a"); // 500k
    const tracker = new SubsidiaryBudgetTracker({ sessionsRepo: repo, readUsage: readUsageStub, now: () => NOW });
    const st = tracker.status({ id: "sub-a", daily_token_budget: 0 }, NOW);
    expect(st.todayTokens).toBe(500_000);
    expect(st.budget).toBe(0);
    expect(st.blocked).toBe(false);
  });

  it("status: 消費 >= 予算で blocked=true", () => {
    addSession("s100", NOW, "sub-a"); // 100k
    const tracker = new SubsidiaryBudgetTracker({ sessionsRepo: repo, readUsage: readUsageStub, now: () => NOW });
    expect(tracker.status({ id: "sub-a", daily_token_budget: 100_000 }, NOW).blocked).toBe(true);
    expect(tracker.status({ id: "sub-a", daily_token_budget: 100_001 }, NOW).blocked).toBe(false);
  });

  it("isOverBudget は status.blocked のショートカット", () => {
    addSession("s100", NOW, "sub-a");
    const tracker = new SubsidiaryBudgetTracker({ sessionsRepo: repo, readUsage: readUsageStub, now: () => NOW });
    expect(tracker.isOverBudget({ id: "sub-a", daily_token_budget: 50_000 }, NOW)).toBe(true);
    expect(tracker.isOverBudget({ id: "sub-a", daily_token_budget: 0 }, NOW)).toBe(false);
  });
});
