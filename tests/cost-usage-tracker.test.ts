...
describe("CostUsageTracker", () => {
  ...
  function makeTracker(opts: {
    ...
    return new CostUsageTracker({
      ...
      now: () => opts.now,
      enumerate: () => opts.files(),
    });
  }

  it("初回サンプルは baseline のみ記録し当日には加算しない", () => {
    let total = 1000;
  ...
  it("2 回目以降は増分 (delta) を当日に加算する", () => {
    let total = 1000;
    const t = makeTracker({ budget: 0, now: T0, files: () => [{ path: "a.jsonl", total }] });
    t.sample();           // baseline 1000
    total = 1700;
    t.sample();           // +700
    total = 2000;
    t.sample();           // +300
  ...
  it("予算超過で blocked=true、 未満なら false、 budget=0 は常に false", () => {
    let total = 0;
    const now = T0;
  ...
  it("外部バッチ (登録外ログ) の増分も加算される — 複数ファイル合算", () => {
