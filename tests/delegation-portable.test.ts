...
describe("GET /v1/delegation/templates/:id/export + POST import", () => {
  ...
  it("export → import で同等の新規テンプレが立つ (call_name は自動採番で衝突回避)", async () => {
    const src = repo.createTemplate({
      ...
      sort_order: 77, category: "parttimer",
    });
    ...
    const created = ((await imp.json()) as { template: { call_name: string; project: string | null; default_cwd: string | null; prompt_template: string; sort_order: number; category: string } }).template;
    ...
    expect(created.category).toBe("parttimer");
  });
