...
describe("claudeCodeProvider", () => {
  it("encodes cwd to projects directory name", async () => {
  ...
  it("resolveSessionId picks env vars", async () => {
  ...
  it("transcriptPath returns null for empty input", async () => {
    expect(await claudeCodeProvider.transcriptPath("", "")).toBeNull();
  });

  it("parseTranscript extracts last assistant text and TodoWrite", async () => {
    const lines = [
    ...
    const r = await claudeCodeProvider.parseTranscript(lines.join("\n"));
  ...
  it("parseTranscript tolerates malformed lines", async () => {
...
describe("codexCliProvider", () => {
  ...
  it("parseTranscript extracts assistant text from Codex JSONL", async () => {
    const lines = [
      JSON.stringify({
        type: "session_meta",
    ...
    const r = await codexCliProvider.parseTranscript(lines.join("\n"));
  ...
  it("parseTranscript tolerates malformed lines", async () => {
    const r = await codexCliProvider.parseTranscript(
      ["{not json}", JSON.stringify({ payload: { role: "assistant", text: "ok" } })].join("\n"),
    );
    expect(r.last_text_summary).toBe("ok");
  });

  it("codexSessionIdFromRecord prefers payload.session_id over payload.id", async () => {
